...
public class persistencia {
	...
	public List<FenomenoUnidade> consultaFenomenoUnidade(String uniid,String fenid)
	{			
	...
	public List<Observacion> consultaObservacion(String fecha, String estid, String tipid, String uniid, String fenid)
	{			
			...
			{
				Observacion tmpArc = dao.queryById(fecha, estid, tipid, uniid, fenid);
				if (tmpArc!=null){
				resultList = new ArrayList();
				resultList.add(tmpArc);	
				}
	...
	//campos basicos est_id, tipo_id, toa,modelodatalog,codigotadalog, std, verionprog,num
	{
		try{
	...
	{
		try{
			List <FenomenoUnidade> fen=this.consultaFenomenoUnidade(campos[3],campos[4]);
			Estacion est=this.consultaEstacionjoin(campos[1], Integer.valueOf(campos[2]));
		em.getTransaction().begin();		
		ObservacionDao dao = new ObservacionDao(em);
